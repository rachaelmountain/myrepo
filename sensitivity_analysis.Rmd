...
SA_MA03 <- bia_all_results(data=readRDS("results_MA03.Rda"))


SA_MA05 <- bia_all_results(data=readRDS("results_MA05.Rda"))


SA_NRT <- bia_all_results(data=readRDS("results_NRT.Rda"))


SA_CDUlow <- bia_all_results(data=readRDS("results_CDUlow.Rda"))


SA_CDUmed <- bia_all_results(data=readRDS("results_CDUmed.Rda"))


SA_CDUhigh <- bia_all_results(data=readRDS("results_CDUhigh.Rda"))
...
bia_SA_combine <- bind_rows(bi_SA_base, bi_SA_NRT, bi_SA_MA03, bi_SA_MA05, bi_SA_CDUlow, bi_SA_CDUmed, bi_SA_CDUhigh) %>% 
  pivot_longer(cols=s10:s3d, names_to="scenario",values_to="CAD") %>% 
  mutate(case=factor(case,levels=c("CDUlow","CDUmed","CDUhigh","base","NRT","MA05","MA03")))
...
p2 <- ggplot(bia_SA_combine %>% filter(cost_group=="bia_case_detection" & scenario!="s10"),
             aes(x=case, y=-CAD/1000000, group=scenario, col=scenario)) +
  scale_color_manual(values=c("#F5594E", "#E7861B", "#95A900", "#00B81F", "#00C0B8", "#00A5FF", "#BF80FF", "#FF61C9")) +
  ylab("Case detection budget impact (million CADs)") +
  geom_point() +
  geom_line() +
  theme_bw()
...
p3 <- ggplot(bia_SA_combine %>% filter(cost_group=="bia_treat" & scenario!="s10"),
             aes(x=case, y=-CAD/1000000, group=scenario, col=scenario)) +
  scale_color_manual(values=c("#F5594E", "#E7861B", "#95A900", "#00B81F", "#00C0B8", "#00A5FF", "#BF80FF", "#FF61C9")) +
  ylab("Treatment budget impact (million CADs)") +
  geom_point() +
  geom_line() +
  theme_bw()
...
p4 <- ggplot(bia_SA_combine %>% filter(cost_group=="bia_hosp" & scenario!="s10"),
             aes(x=case, y=-CAD/1000000, group=scenario, col=scenario)) +
  scale_color_manual(values=c("#F5594E", "#E7861B", "#95A900", "#00B81F", "#00C0B8", "#00A5FF", "#BF80FF", "#FF61C9")) +
  ylab("Hospitalisation budget impact (million CADs)") +
  geom_point() +
  geom_line() +
  theme_bw()
