...
SA_base <- bia_all_results(data=readRDS("results_base_5yrR_fixsim.Rda"))


SA_MA03 <- bia_all_results(data=readRDS("results_MA03_5yrR_fixsim.Rda"))


SA_MA05 <- bia_all_results(data=readRDS("results_MA05_5yrR_fixsim.Rda"))


SA_NRT <- bia_all_results(data=readRDS("results_NRT_5yrR_fixsim.Rda"))


SA_CDUlow <- bia_all_results(data=readRDS("results_CDUl_5yrR_fixsim.Rda"))


#SA_CDUmed <- bia_all_results(data=readRDS("results_CDUmed.Rda"))


SA_CDUhigh <- bia_all_results(data=readRDS("results_CDUh_5yrR_fixsim.Rda"))
...
bia_SA_combine <- bind_rows(bi_SA_base, bi_SA_NRT, bi_SA_MA03, bi_SA_MA05, bi_SA_CDUlow, bi_SA_CDUhigh) %>% 
  ...
  mutate(case=fct_recode(case,"CDU low"="CDUlow","CDU high"="CDUhigh","Base"="base","No NRT"="NRT","50% MA"="MA05","30% MA"="MA03")) %>%  
...
p1 <- ggplot(bia_SA_combine %>% filter(cost_group=="bia_total" & scenario!="S10"),
  ...
  geom_point() +
  geom_line() +
  geom_vline(xintercept="Base", linetype="dashed", color = "darkgrey") +
  theme_bw()
