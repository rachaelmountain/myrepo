...
```{r global settings, echo=FALSE}
  ...
  time_horizon <- 18 
  yrs_btw_CD <- 3 # ensures no individual tested twice
  ...
  p_case_detection <- c(0.1,0.15,0.2,0.25,0.3,0.3,0.3,0.3,0.3,0.3) #c(0.1,0.25,0.4,0.55,0.7) #
...
# * EPIC simulation ------------------------------------------------------------
  ...
  BIA_simul <- function(CD_method, eligibility_criteria){
    ...
    #if(CD_method!="None"){ # can't have this line of code otherwise things like n_agents post CD not counted
...
```{r run simulations, echo=FALSE, message=FALSE}
  ...
  saveRDS(all_s, "results_CDUhigh_repeat3.Rda")
...
# Read in data ------------------------------------------------------------
  ...
  alldat <- readRDS("results_10yr_repeat5.Rda")
  alldat3 <- readRDS("results_10yr_repeat3.Rda") 
...
# * prepare data ----------------------------------------------------------
  ...
  s1a3o <- alldat3$s1a$overall %>% mutate(scenario="s1a3") %>% relocate(scenario)
  s1bo <- alldat$s1b$overall %>% mutate(scenario="s1b") %>% relocate(scenario)
  s1b3o <- alldat$s1b$overall %>% mutate(scenario="s1b3") %>% relocate(scenario)
  s2ao <- alldat$s2a$overall %>% mutate(scenario="s2a") %>% relocate(scenario)
  s2a3o <- alldat3$s2a$overall %>% mutate(scenario="s2a3") %>% relocate(scenario)
  s3bo <- alldat$s3b$overall %>% mutate(scenario="s3b") %>% relocate(scenario)
  s3b3o <- alldat3$s3b$overall %>% mutate(scenario="s3b3") %>% relocate(scenario)

  
  all_overall <- bind_rows(s10o,s1ao,s1a3o,s1bo,s1b3o,s2ao,s2a3o,s3bo,s3b3o) # combine
  ...
  s1a3 <- alldat3$s1a$results %>% mutate(scenario="s1a3", .after=year)
  s1b <- alldat$s1b$results %>% mutate(scenario="s1b", .after=year)
  s1b3 <- alldat3$s1b$results %>% mutate(scenario="s1b3", .after=year)

  s2a <- alldat$s2a$results %>% mutate(scenario="s2a", .after=year)
  s2a3 <- alldat3$s2a$results %>% mutate(scenario="s2a3", .after=year)
  
  s3b <- alldat$s3b$results %>% mutate(scenario="s3b", .after=year)
  s3b3 <- alldat3$s3b$results %>% mutate(scenario="s3b3", .after=year)

  
  
  # combine all year-by-year results
  all_results <- bind_rows(s10,s1a,s1a3,s1b,s1b3,s2a,s2a3,s3b,s3b3) %>% 
...
bia_s10_s1a3 <- bia_table(all_results,"s10","s1a3")
bia_s10_s1b <- bia_table(all_results,"s10","s1b")
bia_s10_s1b3 <- bia_table(all_results,"s10","s1b3")
bia_s10_s2a <- bia_table(all_results,"s10","s2a")
bia_s10_s2a3 <- bia_table(all_results,"s10","s2a3")
bia_s10_s3b <- bia_table(all_results,"s10","s3b")
bia_s10_s3b3 <- bia_table(all_results,"s10","s3b3")
...
bia_all_long <- bind_rows(bia_s10_s1a,
                          ...
                          bia_s10_s3b3) %>% 
  distinct() %>% 
  pivot_longer(cols=year_0:year_10,names_to="year",values_to="CAD") %>% 
  ...
  #mutate(scenario=fct_recode(scenario,"S10"="s10","S1a"="s1a","S13"="s1a3","S1b"="s1b","S1b3"="s1b3","S2a"="s2a","S2a3"="s2a3","S3b"="s3b","S3b3"="s3b3"))
...
p5 <- ggplot(bia_all_long %>% filter(cost_group=="bia_total" & scenario!="s10"),
  ...
  ylab("Total budget impact (million $)") + xlab("Year") +
  geom_point() +
  geom_line() +
  theme_bw()
p6 <- ggplot(bia_all_long %>% filter(cost_group=="bia_case_detection" & scenario!="s10"),
             aes(x=year,y=CAD*(-1)/1000000,group=scenario,col=scenario)) +
  scale_color_manual(values=c("#F5594E", "#E7861B", "#95A900", "#00B81F", "#00C0B8", "#00A5FF", "#BF80FF", "#FF61C9"), name="Scenario") +
  ylab("Case detection budget impact (million $)") + xlab("Year") +
  geom_point() +
  geom_line() +
  theme_bw()
p7 <- ggplot(bia_all_long %>% filter(cost_group=="bia_treat" & scenario!="s10"),
             aes(x=year,y=CAD*(-1)/1000000,group=scenario,col=scenario)) +
  scale_color_manual(values=c("#F5594E", "#E7861B", "#95A900", "#00B81F", "#00C0B8", "#00A5FF", "#BF80FF", "#FF61C9"), name="Scenario") +
  ylab("Treatment budget impact (million $)") + xlab("Year") +
  geom_point() +
  geom_line() +
  theme_bw()
p8 <- ggplot(bia_all_long %>% filter(cost_group=="bia_hosp" & scenario!="s10"),
             aes(x=year,y=CAD*(-1)/1000000,group=scenario,col=scenario)) +
  scale_color_manual(values=c("#F5594E", "#E7861B", "#95A900", "#00B81F", "#00C0B8", "#00A5FF", "#BF80FF", "#FF61C9"), name="Scenario") +
  ylab("Hospitalisation budget impact (million $)") + xlab("Year") +
  geom_point() +
  geom_line() +
...
#   pivot_longer(cols=S1a:S3b3,names_to="scenario",values_to="bia") %>% 
...
all_results_ho_long <- all_results_ho %>%
  pivot_longer(cols=year_0:year_10,names_to="year",values_to="count") %>% 
  #mutate(year=fct_recode(year,"2021"="year_0","2022"="year_1","2023"="year_2","2024"="year_3","2025"="year_4","2026"="year_5"))
  mutate(year=fct_recode(year,"2021"="year_0","2022"="year_1","2023"="year_2","2024"="year_3","2025"="year_4","2026"="year_5",
                         "2027"="year_6","2028"="year_7","2029"="year_8","2030"="year_9","2031"="year_10")) %>%
  mutate(year=factor(year,levels=c("2021","2022","2023","2024","2025","2026","2027","2028","2029","2030","2031")))
