...
```{r global settings, echo=FALSE}
  ...
  settings$n_base_agents <- 18000000 #18e+6 # change number of agents - 18mil=est Canadian pop >= 40, 1st Jan 2015     # 18162236 https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=1710000501
...
# * EPIC simulation ------------------------------------------------------------
  ...
  bia_table <- function(table,base_scenario,alt_scenario){
    ...
    base_table <- table %>%
      ...
      mutate(cost_other=cost_total-cost_case_detection-cost_treat-cost_hosp-cost_maint)
...
```{r run simulations, echo=FALSE, message=FALSE}
  ...
  saveRDS(all_s, "results_base333_5yrR_fixsim.Rda")
...
# Read in data ------------------------------------------------------------
  ...
  alldat <- readRDS("results_base_5yrR_fixsim.Rda")
...
p5 <- ggplot(bia_all_long %>% filter(cost_group=="bia_total" & scenario!="s10"),
  ...
  ylab("Additional total cost (million $)") + xlab("Year") +
  geom_point() +
  geom_line() +
  theme_bw()
p6 <- ggplot(bia_all_long %>% filter(cost_group=="bia_case_detection" & scenario!="s10"),
             aes(x=year,y=CAD*(-1)/1000000,group=scenario,col=scenario)) +
  scale_color_manual(values=c("#F5594E", "#E7861B", "#95A900", "#00B81F", "#00C0B8", "#00A5FF", "#BF80FF", "#FF61C9"), name="Scenario") +
  ylab("Additional case detection costs (million $)") + xlab("Year") +
  geom_point() +
  geom_line() +
  theme_bw()
p7 <- ggplot(bia_all_long %>% filter(cost_group=="bia_treat" & scenario!="s10"),
             aes(x=year,y=CAD*(-1)/1000000,group=scenario,col=scenario)) +
  scale_color_manual(values=c("#F5594E", "#E7861B", "#95A900", "#00B81F", "#00C0B8", "#00A5FF", "#BF80FF", "#FF61C9"), name="Scenario") +
  ylab("Additional treatment costs (million $)") + xlab("Year") +
  geom_point() +
  geom_line() +
  theme_bw()
p8 <- ggplot(bia_all_long %>% filter(cost_group=="bia_hosp" & scenario!="s10"),
             aes(x=year,y=CAD*(-1)/1000000,group=scenario,col=scenario)) +
  scale_color_manual(values=c("#F5594E", "#E7861B", "#95A900", "#00B81F", "#00C0B8", "#00A5FF", "#BF80FF", "#FF61C9"), name="Scenario") +
  ylab("Additional hospitalisation costs (million $)") + xlab("Year") +
  geom_point() +
  geom_line() +
...
mean(c(abs(bia_total$S1a[7:12])[1]/sum(abs(bia_total$S1a[c(7,8,9,10,12)]))*100,
abs(bia_total$S1b[7:12])[1]/sum(abs(bia_total$S1b[c(7,8,9,10,12)]))*100,
abs(bia_total$S1c[7:12])[1]/sum(abs(bia_total$S1c[c(7,8,9,10,12)]))*100,
abs(bia_total$S2a[7:12])[1]/sum(abs(bia_total$S2a[c(7,8,9,10,12)]))*100,
abs(bia_total$S3a[7:12])[1]/sum(abs(bia_total$S3a[c(7,8,9,10,12)]))*100,
abs(bia_total$S3b[7:12])[1]/sum(abs(bia_total$S3b[c(7,8,9,10,12)]))*100,
abs(bia_total$S3c[7:12])[1]/sum(abs(bia_total$S3c[c(7,8,9,10,12)]))*100,
abs(bia_total$S3d[7:12])[1]/sum(abs(bia_total$S3d[c(7,8,9,10,12)]))*100))
